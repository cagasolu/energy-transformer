...
def load_dataset_for_anomaly(class_id=0, diff_samples=10000):
    """
        ...
        Args:
            ...
            diff_samples (int): Maximum number of anomaly samples
            to return. Default is 10000 (set to None to use all 7200)
...
def _mean_roc(roc_list, n_grid=200, smooth=1.5):
    """
        Aggregate multiple per-class ROC curves onto a common FPR
        grid: linearly interpolate each TPR curve, average them, lightly
        Gaussian-smooth the mean, and clamp the endpoints to (0,0) and (1,1).
...
def _norm_within_class(conclusions, method, label_domain, pct=(1, 99)):
    """
        Min-max normalise scores within each class before pooling
        across classes, so that per-class score ranges become
        comparable on a single [0, 1] axis.
        ...
        Returns:
            1D array of pooled, within-class-normalised
            scores.
    """
    pooled = []
...
def plot_anomaly_eval_graphs(conclusions, save_prefix="fig_eval", out_dir="figures"):
    """
        Produce the three evaluation figures from the
        per-class conclusions and save each as PNG + PDF:
          1. grouped AUROC bar chart comparing Energy vs
          Mahalanobis (with means),
          2. side-by-side mean ROC curves (per-class faint
          lines + ±1 std band),
          3. within-class-normalised score distributions
          for normal vs anomaly.

        Args:
            conclusions: list of per-class dicts
            holding 'energy' and 'mahalanobis' metrics.
            save_prefix: filename prefix for the saved figures.
            out_dir: directory the figures are written to (created
            if missing).
    """
    _apply_style()
...
def evaluate_mahalanobis(model_three, normal_loader, anomaly_loader, device, mean, cov_inv):
    """
        Args:
            ...
            anomaly_loader: dataloader for the anomalies.
            cov_inv: latent inverse covariance.
            mean: latent class mean.
            device: torch device.
        Returns:
