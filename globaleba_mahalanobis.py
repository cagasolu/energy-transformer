...
globaleba_mahalanobis.py
     ...
     GLOBAL eba: global Energy Based Attention
         ...
         Mahalanobis Distance requires pretrained weights from pretrain_selyne_recon.py
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.patch_to_pixels = nn.Sequential(
            nn.Linear(embed_dim, 16 * 16 * num_channels)
        )
    ...
    def forward(self, latent_vector: torch.Tensor, encoder_features: torch.Tensor) -> torch.Tensor:
        ...
        patch_grid = patch_pixels.reshape(
            batch_size, self.num_patches, 16, 16, self.num_channels
        )
...
class SELYNE(nn.Module):
    """
        Full Global-EBA encoder–decoder backbone.

        Pipeline:
        multi-scale patch embedding → prepend a CLS token and add
    ...
    def compute_loss(self, zeta, Z_reconstruction):
        ...
        weights = torch.ones(4, device=comp.device)

        return (weights.unsqueeze(0) * comp).sum(dim=1).mean()
...
class FlatImageDataset(Dataset):
    """
        Return one sample. The (random) transform is applied here
        at access time, so a fresh augmentation is sampled every
        epoch. Returns just the image when no labels were
        provided, otherwise an (image, label) pair.
    """
...
def load_dataset_for_anomaly(class_id=0, diff_samples=10000):
    """
        Load CI-FAR-10 for anomaly detection with one-class classification setup.

        Args:
            class_id (int): The class label_set to treat as normal (0-9 for CI-FAR-10)
            diff_samples (int): Maximum number of anomaly samples to return.
                               Default is 5000 (set to None to use all 9000)

        Returns:
            tuple: (train_norm, test_norm, test_anom)
                - train_norm: List of flattened tensors from normal class (5000 samples)
                - test_norm: List of flattened tensors from normal class (1000 samples)
                - test_anom: List of flattened tensors from anomaly classes
    """

    print(f"Loading CI-FAR-10 with normal class: {class_id}")

    # Transform pipeline: resize to 64x64, convert to tensor, normalize with ImageNet stats
    ...
    train_dataset_dist = CIFAR10(root="./data", train=True, download=True, transform=transform_dist)
    test_dataset = CIFAR10(root="./data", train=False, download=True, transform=transform_dist)

    # Filter training set: keep only images from the normal class (5000 samples)

    train_norm = [img_value.reshape(-1) for img_value, label_set in train_dataset_dist if label_set == class_id]

    # Filter test set: separate normal vs anomaly based on class_id
    ...
    # 9000 samples; Cap anomaly samples if specified

    if diff_samples is not None and len(test_anom) > diff_samples:
        test_anom = test_anom[:diff_samples]

    # Print dataset statistics - FIXED: added closing parentheses
...
def _mean_roc(roc_list, n_grid=200, smooth=1.5):
    """
        Aggregate multiple per-class ROC curves onto a common FPR grid: linearly
        interpolate each TPR curve, average them, lightly Gaussian-smooth the mean,
        and clamp the endpoints to (0,0) and (1,1).

        Args:
            roc_list: list of (fpr, tpr) arrays, one per class.
            n_grid: number of FPR grid points.
            smooth: Gaussian smoothing sigma for the mean curve.
        Returns:
            (grid, mean_tpr, std_tpr) arrays over the shared FPR grid.
    """
...
def _norm_within_class(conclusions, method, label_domain, pct=(1, 99)):
    """
        Min-max normalise scores within each class before pooling across classes, so that
        per-class score ranges become comparable on a single [0, 1] axis.

        Args:
            conclusions: list of per-class result dicts.
            method: 'energy' or 'mahalanobis'.
            label_domain: 'normal' or 'anomaly'.
        Returns:
            1D array of pooled, within-class-normalised scores.
    """
    pooled = []
    ...
    for r in conclusions:
        ...
        vals = n if label_domain == 'normal' else a

        pooled.append(np.clip((vals - lo) / rng, 0.0, 1.0))

    return np.concatenate(pooled)


def plot_anomaly_eval_graphs(conclusions, save_prefix="fig_eval", out_dir="figures"):
    """
        Produce the three evaluation figures from the per-class conclusions and save each
        as PNG + PDF:
          1. grouped AUROC bar chart comparing Energy vs Mahalanobis (with means),
          2. side-by-side mean ROC curves (per-class faint lines + ±1 std band),
          3. within-class-normalised score distributions for normal vs anomaly.

        Args:
            conclusions: list of per-class dicts holding 'energy' and 'mahalanobis' metrics.
            save_prefix: filename prefix for the saved figures.
            out_dir: directory the figures are written to (created if missing).
    """
    _apply_style()
    ...
    e_auroc = np.array([r['energy']['auroc'] for r in conclusions])
    ...
    b1 = ax.bar(x_coord - width / 2, e_auroc, width, color=COLORS['energy'],
    ...
    ax.axhline(e_auroc.mean(), color=COLORS['energy'], linestyle='--', linewidth=1.4, alpha=0.85)
    ...
    handles = [
        mpatches.Patch(facecolor=COLORS['energy'], edgecolor='black', label='Energy'),
        mpatches.Patch(facecolor=COLORS['mahal'], edgecolor='black', hatch='//', label='Mahalanobis'),
        plt.Line2D([0], [0], color=COLORS['energy'], ls='--', label=f'Energy mean ({e_auroc.mean():.3f})'),
    ...
    ax.set_ylabel('AUROC')
    ax.set_title('Anomaly Detection AUROC: Energy vs Mahalanobis', fontweight='bold')
    ...
    for ax, method, color, title in [
        (axe, 'energy', COLORS['energy'], '(a) Energy-Based ROC'),
    ...
    for ax, method, title in [
        (axe, 'energy', '(a) Energy Score Distribution'),
...
def evaluate_mahalanobis(model_three, normal_loader, anomaly_loader, device, mean, cov_inv):
    """
        Score normal and anomaly samples by their latent Mahalanobis distance, then
        report detection quality: AUROC, plus accuracy at the Youden-optimal threshold
        (argmax of TPR − FPR).

        Args:
            model_three: SELYNE encoder.
            normal_loader / anomaly_loader: DataLoaders for the two classes.
            device: torch device.
            mean, cov_inv: latent Gaussian statistics from compute_mahalanobis_stats.
        Returns:
            Dict with auroc, acc, fpr, tpr, and
            the raw normal/anomaly distance arrays.
            :param model_three:
            :param normal_loader:
    ...
    return {'auroc': float(auroc), 'fpr': fpr, 'tpr': tpr,
            'normal': d_normal, 'anomaly': d_anomaly}


@torch.no_grad()
def evaluate_energy(model_forth, normal_loader, anomaly_loader, device):
    """
        Score normal and anomaly samples by reconstruction energy and report AUROC plus
        accuracy at the supplied (GPD-calibrated) threshold.

        Args:
            model_forth: SELYNE model.
            normal_loader / anomaly_loader: DataLoaders for the two classes.
            device: torch device.
            threshold: energy decision threshold from calibrate_gpd.
        Returns:
            Dict with auroc, acc, fpr, tpr, and
            the raw normal/anomaly energy arrays.
            :param model_forth:
            :param normal_loader:
            :param anomaly_loader:
            :param device:
    ...
    return {'auroc': float(auroc), 'fpr': fpr, 'tpr': tpr,
            'normal': e_normal, 'anomaly': e_anomaly}


def train_mahalanobis_enhanced(model_five, train_loader_class, device, epochs=80, lr=1e-2, wd=1e-4, lambda_init=1e-0,
                               max_recal=6, attempt=0.0, lambda_used=0.0):
    """
        Args:
            model_five: pretrained SELYNE model.
            train_loader_class: DataLoader of the single normal class.
            device: torch device.
            epochs, lr, wd: optimisation hyperparameters.
            lambda_init: starting lambda each epoch resets to before self-calibrating.
            max_recal: max re-calibrations per epoch (guards against endless oscillation).
        Returns:
            ...
            :param max_recal:
    """
    for param in model_five.parameters():
    ...
    target_mean, target_cov_inv = compute_mahalanobis_stats(model_five, train_loader_class, device)
    ...
    for epoch in range(epochs):
        ...
        # each epoch calibrates from scratch starting at
        # lambda_init, independent of the previous epoch

        lambda_mahal = lambda_init

        # Snapshot taken ONCE at the start of the
        # epoch. Every attempt restarts from here.

        model_state = copy.deepcopy(model_five.state_dict())
        optim_state = copy.deepcopy(optimizer.state_dict())

        cpu_rng = torch.get_rng_state()

        cuda_rng = torch.cuda.get_rng_state(device) if is_cuda else None

        avg_energy = None
        mahal_pre = None
        mahal_post = None

        # keep restarting from the second
        # lambda until it stops changing

        for attempt in range(max_recal):

            lambda_used = lambda_mahal

            # the lambda the PRE pass is about to use
            # the following code -> PRE pass: dry run
            # with the current lambda, just to measure
            # mahal

            model_five.load_state_dict(copy.deepcopy(model_state))
            ...
            for xorn in train_loader_class:
                xorn = xorn.to(device)
                recon, zeta = model_five(xorn)

                energy_loss = model_five.compute_loss(xorn, recon)
                mahal_loss = mahalanobis_distance(zeta, target_mean, target_cov_inv).mean()

                loss_input = energy_loss + lambda_used * mahal_loss
            ...
            mahal_pre = total_mahal / n_batches

            # second lambda_mahal: power of 10 that
            # pulls mahal into the [0.1, 1) band

            digits = len(str(int(mahal_pre)))
            lambda_mahal = 10 ** (-digits)

            # POST pass: real run from the SAME
            # start, using the second lambda
            ...
            for xorn in train_loader_class:
                ...
                loss_input = energy_loss + lambda_mahal * mahal_loss
            ...
            mahal_post = total_mahal / n_batches

            avg_energy = total_energy / n_batches

            # If the second lambda equals the one PRE used, lambda has converged:
            # PRE and POST ran with the same lambda, so mahal_pre == mahal_post ->
            # accept. Otherwise, lambda_mahal already holds the new second lambda,
            # so the loop restarts the calibration FROM that second lambda.

            if lambda_mahal == lambda_used:
                break
        ...
        print(f"  attempt={attempt} broke={lambda_mahal == lambda_used} "
              f"  lambda_used={lambda_used} lambda_mahal={lambda_mahal}")

        print(f"Ep {epoch:3d} | Energy: {avg_energy:.4f} | MahalPre: {mahal_pre} | "
              f"MahalPost: {mahal_post} | "
              f"Total: {avg_energy + lambda_mahal * mahal_post:.4f} | LR: {lr_now:.2e}")
...
def save_reconstructions(model_sixth, loader, device, out_dir="reconstructions",
                         max_samples=10, show_metrics=True):
    """
    ...
    # Randomly pick which dataset indices to reconstruct
    ...
    for x_input_0 in loader:
        ...
        for i in range(x_input_0.shape[0]):
            ...
            # Track global index, skip anything not randomly selected
            ...
            # Create comparison image (original | reconstruction)
            ...
            # Also save individual original and reconstruction
...
if __name__ == "__main__":
    ...
    for CLASS_ID in CLASS_LIST:
        ...
        model = train_mahalanobis_enhanced(model, train_loader, DEVICE, epochs=5, lr=1e-5)
    ...
    plot_anomaly_eval_graphs(waitingTop_results, save_prefix="cifar10_eval")

    print("FINAL RESULTS SUMMARY")

    for result in waitingTop_results:
        print(f"Class {result['class_id']}: "
              f"Energy={result['energy']['auroc']:.4f}, "
              f"Mahalanobis={result['mahalanobis']['auroc']:.4f}")