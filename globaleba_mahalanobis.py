...
class SELYNE(nn.Module):
    """
        Full Global-EBA encoder–decoder backbone. Sign: globaleba_mahalanobis.py

        Pipeline: (for globaleba_mahalanobis.py)
...
def load_dataset_for_anomaly(class_id=0, diff_samples=10000):
    """
        Load STL-10 for anomaly detection with one-class classification setup.

        Args:
            class_id (int): The class label_set to treat as normal (0-9 for STL-10)
            diff_samples (int): Maximum number of anomaly samples to return.
                               Default is 10000 (set to None to use all 7200)

        Returns:
            tuple: (train_norm, test_norm, test_anom)
                - train_norm: List of flattened tensors from normal class (500 samples)
                - test_norm: List of flattened tensors from normal class (800 samples)
    ...
    # Download and load STL-10 datasets
...
def augment_batch(x, img_size, in_chans):
    """
        Light, tensor-based augmentation to produce two views.
    """
    B = x.shape[0]

    ImgMeetHall = x.view(B, in_chans, img_size, img_size)
    flip = (torch.rand(B, device=x.device) < 0.5).view(B, 1, 1, 1)

    ImgMeetHall = torch.where(flip, ImgMeetHall.flip(-1), ImgMeetHall)
    sh, sw = random.randint(-2, 2), random.randint(-2, 2)

    # Reduced from [-4, 4]

    ImgMeetHall = torch.roll(ImgMeetHall, shifts=(sh, sw), dims=(-2, -1))

    ImgMeetHall = ImgMeetHall + 0.001 * torch.randn_like(ImgMeetHall)
...
def latent_consistency(z_s, z_t):
    """
        BYOL-style with variance regularization to prevent collapse.
    """
    z_s = F.normalize(z_s, dim=-1)
    z_t = F.normalize(z_t, dim=-1)

    # Consistency loss with stop-gradient

    consistency = 2 - 2 * (z_s * z_t.detach()).sum(dim=-1).mean()
...
def train_mahalanobis_enhanced(model_five,
    ...
    """
        Args:
            ...
            train_loader_class: DataLoader of the single
            normal class. device: torch device. epochs, lr,
            wd, ema_decay: optimisation hyperparameters.
        Returns:
    ...
    for epoch in range(epochs):
        ...
        for xorn in train_loader_class:
            ...
            consist_loss = latent_consistency(z_s, z_t)

            loss_input = 5e-1 * energy_loss + (math.pi / 10) * consist_loss
...
if __name__ == "__main__":
    ...
    for CLASS_ID in CLASS_LIST:
        ...
        model = train_mahalanobis_enhanced(model, train_loader, DEVICE, epochs=10, lr=1e-4)
    ...
    for result in waitingTop_results:
        print(f"Class {result['class_id']}: "
              f"Energy AUROC={result['energy']['auroc']:.4f}, "
