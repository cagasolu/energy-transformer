...
pretrain_selyne_recon.py
     ...
     GLOBAL eba: global Energy Based Attention
         ...
         —————————————————————————————————————
         SELYNE:    Stable-Energy Lipschitz Net
         ...
         Network use is distribution: if you run a modified 
         version of this program and let users interact with 
         it over a network, you must make the corresponding 
         source available to them (AGPLv3 §13).
        
         Commercial licensing (closed-source API, proprietary 
         product, embedded device): cangorkemsu@gmail.com



->>  Uses ImageNet Subset (all INTEGER classes)
->>  Model: Global EBA (encoder + decoder)
            ->>  precise architecture

->>  Output: full model weights (pretrained_selyne_full_recon.pt)
...
class EnergyBasedAttention(nn.Module):
    """
        Multi-head self-attention with a
        learned *energy* (bilinear) compatibility
        function instead of a plain dot product.

        For each head, query-key affinity is
        computed as a bilinear form Qᵀ·M·K,
        where M is a learnable (orthogonally
        initialised) per-head matrix, and
        the scores are scaled by a learnable
        per-head temperature before the
        softmax.

        This lets the model learn
        a richer, head-specific notion
        of token similarity than standard
    ...
    def __init__(self, dim, num_heads=8, dropout=0.1):
        ...
        self.norm_scale = nn.Parameter(torch.ones(num_heads))
    ...
    def forward(self, zone):
        """
            Project the input into Q, K, V;
            compute per-head bilinear energy
            scores (Q·M·Kᵀ), divide by the
            learned temperature, softmax into
            attention weights, aggregate V,
            then apply the output projection.

            Args:
                zone: input tokens of shape
                (B, N, D).
            Returns:
        ...
        proj = self.W_tied(zone)

        # (B, N, D)

        Q = proj.reshape(B, N, H, Dh).permute(0, 2, 1, 3)

        # (B, H, N, Dh)

        K = proj.reshape(B, N, H, Dh).permute(0, 2, 1, 3)
        ...
        MK = torch.einsum('hde,bhke->bhkd', self.M, K)
        rational_mk = F.normalize(MK, dim=-1)

        immediate_conclusion = rational_mk * self.norm_scale.view(1, H, 1, 1).abs()

        score = torch.einsum('bhqd,bhkd->bhqk', Q, immediate_conclusion)
...
class TransformerBlock(nn.Module):
    """
        ...
        LayerNorm → EnergyBasedAttention →
        residual,

        then LayerNorm → GELU MLP →
        residual.

        The attention sublayer uses the
        energy-based attention defined
        above rather than vanilla
...
class SELYNE(nn.Module):
    """
        Full Global-EBA encoder–decoder backbone. Sign: pretrain_selyne_recon.py

        Pipeline: (for pretrain_selyne_recon.py)
...
def test_proposition5(modeled):
    """
        Check whether M_h matrices
        are asymmetric/indefinite.

        Proposition 5 states:
        - If M_h ∈ Sym_+ (positive
          semidefinite symmetric),
          then the bilinear form can
          be reproduced by standard
          tied attention.
        - If M_h ∉ Sym_+ (asymmetric
          or indefinite), then tied
          Gloeba offers greater
          figurative power than
          standard tied attention.

        Args:
            modeled: SELYNE model
            with tied Gloeba attention
    """

    for idx, (name, param) in enumerate(modeled.named_parameters()):
        if 'M' in name and 'proto' not in name:
            M = param.detach().cpu()

            asym = torch.norm(M - M.transpose(-2, -1))
            ...
            has_index = torch.any(eigenvalues < -1e-6)

            if is_psd and not has_asym:
                territory = "standard tied attention"
            elif has_asym and not has_index:
                territory = "asymmetric psd"
            elif has_index and not has_asym:
                territory = "indefinite symmetric (krein space)"
            elif has_asym and has_index:
                territory = "asymmetric + indefinite (krein space)"
            else:
                territory = "tied gloeba territory"

            print(f"Head {idx}:")
            print(f"  Asymmetric norm: {asym:.6f}")
            print(f"  Min eigenvalue: {eigenvalues.min():.6f}")
            print(f"  Max eigenvalue: {eigenvalues.max():.6f}")
            print(f"  In Sym_+: {is_psd.item()}")
...
def plot_professional_graphs(
    ...
    PLOTS_DIR = "/content/drive/MyDrive/pre_training_plots"
...
if __name__ == "__main__":
    ...
    for ep in range(EPOCHS):
        ...
        if val_acc > best_acc:
            ...
            torch.save(model.state_dict(), "/content/drive/MyDrive/pretrained_selyne_full_recon_best.pt")
    ...
    print("\nAfter Training, Tied Gloeba Territory Analysis:")
    test_proposition5(model)

    # Save final model

    torch.save(model.state_dict(), "/content/drive/MyDrive/pretrained_selyne_full_recon.pt")

    print(f"Pretraining finished → pretrained_selyne_full_recon.pt (best val_acc: {best_acc:.3f})")
    ...
    print("\n   The figures were saved successfully into the pre_training_plots folder!")
