...
energetic_mahal_science.py
     ...
     GLOBAL eba: global Energy Based Attention
         ...
         2.) One-dimensional version of globaleba_mahalanobis.py:
             ->>  globaleba_mahalanobis.py  (convolution calculation methodology)
...
def load_dataset_for_anomaly(data_root="./data/brisc2025/brisc2025/classification_task", diff_samples=10000):
    """
        ...
        Args:
            data_root (str): Path to the 
            dataset folder (train/no_tumor, 
            test/*).
            diff_samples (int): Max number 
            of anomaly samples to include.

        Returns:
            tuple: (train_normal, 
            test_normal, test_anomaly) 
            as lists of torch tensors.
    """
...
if __name__ == "__main__":
    ...
    param_seed_values = [0, 1, 5, 13, 89, 144, 2584]
    ...
    for param_seed_value in param_seed_values:
        print(f"\n")
        print(f"SEED = {param_seed_value}")
        ...
        checkpoint_path = "pretrained_selyne_full_recon.pt"
        ...
        print(f"FINAL RESULTS (Brain Tumor MRI) - Seed {param_seed_value}")
        print(f"Reconstruction Score AUROC: {score_results['auroc']:.4f}")

        print(f"Mahalanobis AUROC: {mahalanobis_results['auroc']:.4f}")

        ckpt_name = f"selyne_brain_tumor_seed{param_seed_value}.pt"
        ...
        print(f"\n✓ Model saved: {ckpt_name}")

        save_reconstructions(model, test_normal_loader, DEVICE,
                             out_dir=f"recon_brain_normal_seed{param_seed_value}", show_metrics=True)

        save_reconstructions(model, test_anomaly_loader, DEVICE,
                             out_dir=f"recon_brain_anomaly_seed{param_seed_value}", show_metrics=True)

        all_results.append({
            'seed': param_seed_value,
            'score_auroc': score_results['auroc'],
            'mahalanobis_auroc': mahalanobis_results['auroc'],
        })
    ...
    score_auroc_s = [r['score_auroc'] for r in all_results]
    mahal_auroc_s = [r['mahalanobis_auroc'] for r in all_results]

    for r in all_results:
        print(f"Seed {r['seed']:>5} | Score AUROC: {r['score_auroc']:.4f} | "
              f"Mahalanobis AUROC: {r['mahalanobis_auroc']:.4f}")

    print(f"\nScore AUROC:       {np.mean(score_auroc_s):.4f} ± {np.std(score_auroc_s):.4f}")
    print(f"Mahalanobis AUROC: {np.mean(mahal_auroc_s):.4f} ± {np.std(mahal_auroc_s):.4f}")

# Brisc2025 experiments deal with 
# the Tied Gloeba Codex; no graphic 
# drawing, just calculations.